...
def loss(learning_rate):
	...
	#MSE_loss = tf.scalar_mul(0.5, tf.reduce_mean(tf.pow(tf.subtract(y_pred, y_target), 2)))
...
def plot():
	...
	fig, ax = plt.subplots()
	ax.plot(training_loss_1, 'k--', label = "training rate: 0.005")
	ax.plot(training_loss_2, 'k:', label = "training rate: 0.001")
	ax.plot(training_loss_3, 'k', label = "training rate: 0.0001")
	legand = ax.legend(loc="upper center", shadow=True)
