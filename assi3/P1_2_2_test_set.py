...
def plot(learning_rate):
	test_error_list_2_layer= loss(learning_rate)
	tf.reset_default_graph() 
	test_error_list_1_layer= loss2(learning_rate)
	plt.figure()
	plt.plot(test_error_list_2_layer)
	plt.plot(test_error_list_1_layer)
	plt.legend(['2 hidden layers', '1 hidden layer'], loc='upper right')
	plt.title("1.2.2 Test set error")
	print("1.2.2 Final test set error for 2 hidden layers:%f, Final test set error for 1 hidden layer:%f."%(test_error_list_2_layer[-1],test_error_list_1_layer[-1]))
	plt.xlabel("Epoch")
	plt.ylabel("Error")
	plt.grid(True)
	plt.show()
