...
def loss(learning_rate):
	...
	with tf.variable_scope("hidden_layer1"):
		...
		W_X_1_dropout = tf.nn.dropout(data, 0.5)
...
##############################################################
	...
	with tf.variable_scope("hidden_layer1_nodo"):
		W_X_1_nodo = add_one_layer(data,1000)
		tf.get_variable_scope().reuse_variables()
		weight1_nodo=tf.get_variable("weights")

	with tf.variable_scope("output_layer_nodo"):
		W_X_2_nodo = add_one_layer(tf.nn.relu(W_X_1_nodo),10)
		tf.get_variable_scope().reuse_variables()
		weight2_nodo=tf.get_variable("weights")

	sig_prediction_nodo = tf.nn.softmax(logits=W_X_2_nodo)
	entropy_nodo = tf.nn.softmax_cross_entropy_with_logits(labels=target, logits=W_X_2_nodo)
	entropy_loss_nodo =tf.reduce_mean(entropy_nodo)
	weight_decay_loss1_nodo = decay_efficient * tf.nn.l2_loss(weight1_nodo)
	weight_decay_loss2_nodo = decay_efficient * tf.nn.l2_loss(weight2_nodo)
	weight_decay_loss_nodo = weight_decay_loss1_nodo + weight_decay_loss2_nodo
	loss_nodo = entropy_loss_nodo + weight_decay_loss_nodo
	max_prob_nodo = tf.argmax(sig_prediction_nodo,axis=1)
	optimizer_nodo = tf.train.AdamOptimizer(learning_rate).minimize(loss_nodo)

	sess = tf.Session()
	sess.run(tf.global_variables_initializer())

	W25_nodo = np.zeros((28*28, 1000))
	W100_nodo= np.zeros((28*28, 1000))
	...
	for i in range(epoch):  
		...
		for k in range(batch_num):
			...
			sess.run(optimizer_nodo, feed_dict={data: batch_data, target: batch_target})
		...
		if (i == (epoch-1)*0.25):
			W25_nodo = sess.run(weight1_nodo)
		if (i == epoch-1):
			W100_nodo = sess.run(weight1_nodo)
	...
	return train_loss_list, train_accuracy_list, valid_loss_list, valid_accuracy_list,test_loss_list, test_accuracy_list, W25,W100,W25_nodo,W100_nodo


def plot(learning_rate):
	train_loss_list, train_accuracy_list, valid_loss_list, valid_accuracy_list, test_loss_list, test_accuracy_list, W25,W100,W25_nodo,W100_nodo= loss(learning_rate)
	
	visualization(W25, "dropout25_e25")
	visualization(W100, "dropout100_e100")
	visualization(W25_nodo, "dropout25_e25_nodo")
	visualization(W100_nodo, "dropout100_e100_nodo")
...
def visualization(W, name):
	...
	for X in range(10):
		for Y in range(10):
			for x in range(28):
				for y in range(28):
					figure[Y*(28+2)+y, X*(28+2)+x] = W[y*28+x, 5*Y*10+X]
