...
def loss(learning_rate):
	...
	for i in range(epoch):  
		...
		train_accuracy_list.append(1-np.mean(sess.run(max_prob, feed_dict={data: trainData}) == trainTarget))
		valid_accuracy_list.append(1-np.mean(sess.run(max_prob, feed_dict={data: validData}) == validTarget))
		test_accuracy_list.append(1-np.mean(sess.run(max_prob, feed_dict={data: testData}) == testTarget))
...
def plot(learning_rate):
	train_loss_list, train_accuracy_list, valid_loss_list, valid_accuracy_list, test_loss_list, test_accuracy_list = loss(learning_rate)
	
	
	for i in range (len(valid_loss_list)):
		print("Epoch number: %f, Training Loss: %f."%(i,train_loss_list[i]))

	for i in range (len(valid_accuracy_list)):
		print("Epoch number: %f, Error: %f."%(i,train_accuracy_list[i]))
	...
	plt.title("1.1.2 Classification Error for best learning rate 0.001")
	plt.xlabel("Epoch")
