...
def SGD_train_loss(learning_rate):
	with np.load("notMNIST.npz") as data :
		Data, Target = data ["images"], data["labels"]
		posClass = 2
		negClass = 9
...
def plot():
	...
	#for x in l_r:
		...
		#key = str(x)
		#dicts[key] = loss_v;
	SGD_training = SGD_train_loss(0.001)
	training = train_loss(0.001)
	...
	plt.plot(SGD_training)#,'k', label = "Learning Rate: 0.005")
	...
	plt.legend(['SGD_training Loss','Adam training Loss'], loc='upper right')
	plt.title("2.1.2 Cross Entropy Loss (with learning rate: 0.001)")
	plt.grid(True)
	plt.suptitle("Adam Training VS SGD Training")
