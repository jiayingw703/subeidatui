	...
	for i in range(714): # 5000 iterations / 7 batches = 714 epoch
		...
		for i in list(range(0, len(validation_pred))):
		      if validation_pred[i] > 0.5 and validTarget[i] > 0.5:
		          ...
		          acc_count += 1
		valid_count = acc_count
		valid.append(acc_count/100.0*100.0)	
	return train,valid,train_count,valid_count
...
def plot():
	...
	#for x in l_r:
		...
		#key = str(x)
		#dicts[key] = loss_v;
	training,validation,train_count,valid_count = loss(0.001)
	...
	plt.title("2.1.1 Accuracy VS Epoch(with learning rate: 0.001)")
	plt.ylabel('Accuracy(%)')
	#plt.ylim([90,100])
	plt.grid(True)
	plt.xlabel('Epoch')
